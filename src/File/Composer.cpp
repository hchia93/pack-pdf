...
namespace packpdf
{
    ...
    namespace
    {
        ...
        void CapPageToA4(FPDF_PAGE page)
        {
            ...
            FS_RECTF clip{ 0.0f, 0.0f, static_cast<float>(newW), static_cast<float>(newH) };
            FPDFPage_TransFormWithClip(page, &m, &clip);
            FPDFPage_SetMediaBox(page, 0.0f, 0.0f, static_cast<float>(newW), static_cast<float>(newH));
        ...
        // 1-indexed page-list spec for FPDF_ImportPages: nullopt = skip row,
        // empty = all pages (NULL spec), non-empty = string like "1-3,5".
        ...
        int OfstreamWriteBlock(FPDF_FILEWRITE* pThis, const void* data, unsigned long size)
        {
            ...
            ctx->out->write(static_cast<const char*>(data), static_cast<std::streamsize>(size));
            if (!ctx->out->good())
            {
                ctx->ok = false;
                return 0;
        ...
        // Matrix placing the image's unit square (rotated `rot` degrees CW)
        // into the rect at (x, y) sized (w, h), post-rotation dimensions.
        ImgMatrix MakeImageMatrix(double x, double y, double w, double h, int rot)
        {
            switch (((rot % 360) + 360) % 360)
            {
                ...
                case 90:  return { 0.0, -h,   w,   0.0, x,        y + h };
                case 180: return { -w,  0.0,  0.0, -h, x + w,    y + h };
                case 270: return { 0.0,  h,  -w,   0.0, x + w,    y     };
            }
        }

        bool LoadJpegImageObj(FPDF_DOCUMENT dest, const std::vector<uint8_t>& jpeg, FPDF_PAGEOBJECT* outImg, int* outPxW, int* outPxH, std::string& err)
        {
        ...
        // PNG decode via stb_image, swizzle RGBA→BGRA, hand to PDFium via
        // FPDFImageObj_SetBitmap (deep-copies, so source buffer is freed here).
        bool LoadPngImageObj(FPDF_DOCUMENT dest, const std::vector<uint8_t>& png, FPDF_PAGEOBJECT* outImg, int* outPxW, int* outPxH, std::string& err)
        {
            int w = 0, h = 0, comp = 0;
            unsigned char* rgba = stbi_load_from_memory(png.data(), static_cast<int>(png.size()), &w, &h, &comp, 4);
            if (!rgba || w <= 0 || h <= 0)
            {
                if (rgba)
                {
            ...
            FPDF_BITMAP bm = FPDFBitmap_CreateEx(w, h, FPDFBitmap_BGRA, rgba, w * 4);
            if (!bm)
            {
                stbi_image_free(rgba);
        ...
        bool LoadImageObj(FPDF_DOCUMENT dest, ImageFormat format, const std::vector<uint8_t>& bytes, FPDF_PAGEOBJECT* outImg, int* outPxW, int* outPxH, std::string& err)
        {
            switch (format)
            {
        ...
        ImageInRect ComputeImageInRect(int pxW, int pxH, const ImageOptions& opts, double rectX, double rectY, double rectW, double rectH)
        {
            ImageInRect R{};
            R.rotation = RotationFor(pxW, pxH, opts.orientation, opts.reverse180);

            const double dpi = 100.0;
            double imgNatW, imgNatH;
            if (R.rotation == 90 || R.rotation == 270)
            {
            ...
            const double availW = rectW - (opts.addPadding ? 2.0 * kPaddingPts : 0.0);
            const double availH = rectH - (opts.addPadding ? 2.0 * kPaddingPts : 0.0);
            const double fit = std::min(availW / imgNatW, availH / imgNatH);
            const double s   = (opts.scaleMode == ImageScaleMode::FitPage)
        ...
        bool AppendImagePage(FPDF_DOCUMENT dest, int& insertIndex, const std::vector<uint8_t>& bytes, const ImageOptions& opts, std::string& err)
        {
            FPDF_PAGEOBJECT img = nullptr;
            int pxW = 0, pxH = 0;
            if (!LoadImageObj(dest, opts.format, bytes, &img, &pxW, &pxH, err))
            {
                return false;
            }

            ImagePageLayout L = ComputeImagePageLayout(pxW, pxH, opts);
            ...
            const ImgMatrix m = MakeImageMatrix(L.imgX, L.imgY, L.imgW, L.imgH, L.rotation);
        ...
        bool AppendMergedImagePage(FPDF_DOCUMENT dest, int& insertIndex, const std::vector<uint8_t>& topBytes, const ImageOptions& topOpts, const std::vector<uint8_t>* botBytes, const ImageOptions* botOpts, std::string& err)
        {
            ...
            auto place = [&](const std::vector<uint8_t>& bytes, const ImageOptions& opts, double rectY) -> bool 
            {
                FPDF_PAGEOBJECT img = nullptr;
                int pxW = 0, pxH = 0;
                if (!LoadImageObj(dest, opts.format, bytes, &img, &pxW, &pxH, err))
                ...
                ImageInRect R = ComputeImageInRect(pxW, pxH, opts, 0.0, rectY, pageW, half);
                const ImgMatrix m = MakeImageMatrix(R.imgX, R.imgY, R.imgW, R.imgH, R.rotation);
                FPDFImageObj_SetMatrix(img, m.a, m.b, m.c, m.d, m.e, m.f);
            ...
            if (!place(topBytes, topOpts, half))
            {
                FPDFPage_GenerateContent(page);
                FPDF_ClosePage(page);
                return false;
        ...
        // Returns the ImageOptions stored in the row, or nullptr if it's a PDF row.
        const ImageOptions* AsImageOpts(const TimelineRow& r)
        {
            return std::get_if<ImageOptions>(&r.options);
        }
    ...
    {
        ImagePageLayout L{};
        L.rotation = RotationFor(pxW, pxH, opts.orientation, opts.reverse180);
        ...
        // A4 portrait page; image scaled down to fit (with optional 0.5" margin)
        // and centered. Original keeps native size, FitPage upscales to fill.
        L.pageW = kA4PortraitW;
        L.pageH = kA4PortraitH;
        const double availW = L.pageW - (opts.addPadding ? 2.0 * kPaddingPts : 0.0);
        const double availH = L.pageH - (opts.addPadding ? 2.0 * kPaddingPts : 0.0);
        const double fit = std::min(availW / imgNatW, availH / imgNatH);
        const double s   = (opts.scaleMode == ImageScaleMode::FitPage) ? fit : std::min(1.0, fit);
        L.imgW = imgNatW * s;
        L.imgH = imgNatH * s;
    ...
    ComposeResult ComposeToFile(const TimelineContainer& rows, const std::filesystem::path& outputPath)
    {
        ...
        srcBuffers.reserve(rows.size());
        srcDocs.reserve(rows.size());
        ...
        for (size_t i = 0; i < rows.size(); /* advanced inside */)
        {
            const TimelineRow& row = rows[i];

            std::filesystem::path srcPath = Utf8ToPath(row.path);
            std::string ferr;
            std::vector<uint8_t> bytes = ReadAll(srcPath, ferr);
            if (bytes.empty())
            {
                r.errorMessage = ferr + ": " + row.path;
                cleanup();
                return r;
            }
            ...
            if (auto* pdf = std::get_if<PDFOptions>(&row.options))
            {
                ...
                if (!src)
                {
                    r.errorMessage = "not a valid PDF: " + row.path;
                    cleanup();
                    return r;
                }

                int pageCount = FPDF_GetPageCount(src);
                auto specOpt = BuildImportSpec(pdf->pageSelection, pdf->rangeFirst, pdf->rangeLast, pageCount);
                if (!specOpt.has_value())
                {
                    // Selection resolved to zero pages; keep src alive (writers
                    // may reference it) but import nothing.
                ...
                if (!FPDF_ImportPages(dest, src, specCStr, insertIndex))
                {
                    ...
                    r.errorMessage = "FPDF_ImportPages failed: " + row.path;
                    cleanup();
                    return r;
                }
            ...
            const ImageOptions& opts = std::get<ImageOptions>(row.options);

            // AutoMerge: peek the next row. If it is also a landscape image
            // (any image format works), stack both onto one A4 page.
            bool consumedPair = false;
            if (IsLandscapeMode(opts.orientation) && opts.autoMerge
                && i + 1 < rows.size())
            {
                if (const ImageOptions* nxtOpts = AsImageOpts(rows[i + 1]))
                {
                    if (IsLandscapeMode(nxtOpts->orientation))
                    {
                        std::filesystem::path nxtPath = Utf8ToPath(rows[i + 1].path);
                        std::string ferr2;
                        std::vector<uint8_t> bytes2 = ReadAll(nxtPath, ferr2);
                        if (bytes2.empty())
                        {
                            r.errorMessage = ferr2 + ": " + rows[i + 1].path;
                            cleanup();
                            return r;
                        }
                        std::string ierr;
                        if (!AppendMergedImagePage(dest, insertIndex, bytes, opts, &bytes2, nxtOpts, ierr))
                        {
                            r.errorMessage = ierr + ": " + row.path;
                            cleanup();
                            return r;
                        }
                        ++r.pagesWritten;
            ...
            if (IsLandscapeMode(opts.orientation) && opts.autoMerge)
            {
                // Lonely landscape with autoMerge: render alone in the top
                // half so the user-visible "0.5 page" rule holds.
                std::string ierr;
                if (!AppendMergedImagePage(dest, insertIndex, bytes, opts, nullptr, nullptr, ierr))
                {
                    r.errorMessage = ierr + ": " + row.path;
                    cleanup();
                    return r;
            ...
            std::string ierr;
            if (!AppendImagePage(dest, insertIndex, bytes, opts, ierr))
            {
                r.errorMessage = ierr + ": " + row.path;
                cleanup();
                return r;
    ...
    int GetPdfPageCount(const std::filesystem::path& pdfPath)
    {
        ...
        EnsurePdfiumInit();
        FPDF_DOCUMENT doc = FPDF_LoadMemDocument(buf.data(), static_cast<int>(buf.size()), nullptr);
        if (!doc)
        {
            return -1;
        }
